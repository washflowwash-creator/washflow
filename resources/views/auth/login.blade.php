<x-guest-layout>
    <div class="mb-5 text-center">
        <h1 class="text-2xl font-semibold text-sky-900">Welcome Back</h1>
        <p class="text-sm text-slate-600">Sign in to manage your RBJ Laundry services.</p>
    </div>
    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div>
            ...
            <x-text-input id="email" class="rbj-input block" type="email" name="email" :value="old('email')" required autofocus autocomplete="username" />
        ...
        <div class="block mt-4">
            <label for="remember_me" class="inline-flex items-center">
                <input id="remember_me" type="checkbox" class="rounded border-sky-300 text-sky-600 shadow-sm focus:ring-sky-500" name="remember">
        ...
        <div class="mt-5 flex items-center justify-between gap-3">
            @if (Route::has('password.request'))
                <a class="text-sm text-sky-700 underline" href="{{ route('password.request') }}">
                    {{ __('Forgot your password?') }}
                </a>
            @endif

            <div class="flex gap-2">
                <button class="rbj-btn-primary" type="submit">Log in</button>
                <a href="{{ url('/') }}" class="rbj-btn-outline">← Back</a>
            </div>
        </div>

        <p class="mt-5 text-center text-sm text-slate-600">
            New to RBJ Laundry Shop?
            <a href="{{ route('register') }}" class="font-semibold text-sky-700 underline">Create an account</a>
        </p>
    </form>
</x-guest-layout>
